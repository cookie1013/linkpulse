...
import com.cookie.linkpulse.common.constant.RedisKeyConstants;
import com.cookie.linkpulse.common.constant.SystemConstants;
import com.cookie.linkpulse.common.util.IpUtils;
...
public class ShortLinkServiceImpl implements ShortLinkService {
    ...
    public ShortLinkResponse createShortLink(CreateShortLinkRequest request) {
        ...
        return new ShortLinkResponse(
                ...
                SystemConstants.SHORT_URL_PREFIX + saved.getShortCode()
        );
    }
    ...
    public String getOriginalUrlByShortCode(String shortCode, HttpServletRequest request) {
        ...
        String redisKey = RedisKeyConstants.shortLinkKey(shortCode);
        ...
        event.setClientIp(IpUtils.getClientIp(request));
        event.setUserAgent(request.getHeader(HttpHeaderConstants.USER_AGENT));
        event.setReferer(request.getHeader(HttpHeaderConstants.REFERER));
    ...
    public PageResponse<ShortLinkPageItemResponse> pageQuery(ShortLinkPageQuery query) {
        ...
        List<ShortLinkPageItemResponse> records = page.getContent().stream()
                .map(item -> new ShortLinkPageItemResponse(
                        ...
                        item.getShortCode(),
                        item.getOriginalUrl(),
                        SystemConstants.SHORT_URL_PREFIX + item.getShortCode(),
                        item.getStatus(),
                        item.getPv(),
    ...
    public void disableShortLink(Long id) {
        ...
        String redisKey = RedisKeyConstants.shortLinkKey(shortLink.getShortCode());
    ...
    public ShortLinkStatsDetailResponse getStatsDetail(Long id) {
        ...
        return new ShortLinkStatsDetailResponse(
                ...
                SystemConstants.SHORT_URL_PREFIX + shortLink.getShortCode(),
                shortLink.getStatus(),
                shortLink.getPv(),
    ...
    public List<TopShortLinkItemResponse> listTopLinks(Integer limit) {
        ...
        PageRequest pageRequest = PageRequest.of(
                0,
                size,
                Sort.by(Sort.Direction.DESC, "pv")
        );
    ...
    public List<RefererStatsItemResponse> getRefererStats(Long id) {
        ...
        return rows.stream()
                .map(row -> new RefererStatsItemResponse(
                        row[0] == null || row[0].toString().isBlank() ? "direct" : row[0].toString(),
